...
namespace Modules\RateMargin\Http\Controllers;
...
class RateMarginController extends Controller
{
    ...
    {
        return view('ratemargin::modal.add');
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'holiday_margin' => 'required',
        ]);

        $result = RateMargin::create($input);

        $message = 'Rate Margins Successfully Added';
        if ($result) {
    ...
    public function tableData(Request $request)
    {
        ...
        $query = RateMargin::orderBy($array[$column], $input['order'][0]['dir']);
    ...
    {
        $result = RateMargin::where('id', $id)->first();
        $data['data'] = $result;
        return view('ratemargin::modal.edit')->with($data);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $result = RateMargin::where('id', $id)->update($input);

        $message = 'Rate Margins has successfully updated';
    ...
    {
        $input = $request->all();
        $result = RateMargin::where('id', $input['id'])->first();
        RateMargin::where('id', $result['id'])->delete();
